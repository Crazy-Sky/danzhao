...
import com.danzhao.util.SoundUtil;
import com.jacob.activeX.ActiveXComponent;
import com.jacob.com.ComThread;
import com.jacob.com.Dispatch;
import com.jacob.com.Variant;

/** * 调用MSTTS将字符串转换成语音信息 * * @author liqiang4j */
...
public class MSTTSSpeech {
	...
	* @author cx
	...
	/** * 播放语音 * * @param text * 要转换成语音的文本 * @param type * 类型0:播放，1：停止 */
	...
	/**
	 * * 将文字转换成音频信号，然后输出到.WAV文件 * * @param text * 文本字符串 * @param filePath *
	 * 输出文件路径
	 */
	public void saveToWav(String text, String filePath) { // 创建输出文件流对象
//		System.out.println(text);
		ax = new ActiveXComponent("Sapi.SpFileStream");
		spFileStream = ax.getObject(); // 创建音频流格式对象
		if (spAudioFormat == null) {
			ax = new ActiveXComponent("Sapi.SpAudioFormat");
			spAudioFormat = ax.getObject();
		}
		...
		// 设置音频流格式类型
		Dispatch.put(spAudioFormat, "Type", new Variant(this.formatType));
		// 设置文件输出流的格式 Dispatch.putRef(spFileStream, "Format", spAudioFormat);
		...
		// 调整音量和读的速度 Dispatch.put(spVoice, "Volume", new Variant(this.volume));
		// 设置音量 
	...
	public static void main(String[] args) {
		...
		String text1 = "请，考生号尾号为：一二三四，陈新同学，到考场一五零四参加考试";
		speech.setFormatType(4);
		// speech.speak(text);
	//	speech.setRate(-1);
		speech.saveToWav(text1, "D://MyEclipse安装包及tomocat//tomacat//apache-tomcat-7.0.81//webapps//danzhao2.4//wav//1.wav");
		SoundUtil.wavToPcm("D://MyEclipse安装包及tomocat//tomacat//apache-tomcat-7.0.81//webapps//danzhao2.4//wav//1.wav");
		
		MSTTSSpeech speech2 = new MSTTSSpeech();
		speech2.setFormatType(4);
		String text2 = "请，考生号尾号为：一二三四，陈新同学，到考场一五零四参加考试";
		// speech.speak(text);
	//	speech.setRate(-1);
		speech2.saveToWav(text2, "D://MyEclipse安装包及tomocat//tomacat//apache-tomcat-7.0.81//webapps//danzhao2.4//wav//1.wav");
		SoundUtil.wavToPcm("D://MyEclipse安装包及tomocat//tomacat//apache-tomcat-7.0.81//webapps//danzhao2.4//wav//1.wav");
		
	}
	
	/*public static void main(String[] args){
		 MSTTSSpeech speech1 = new MSTTSSpeech();
		 String text1 = "这是我的测试,物理内存至少需要512MB，建议2GB以上,虚拟内存是主机物理内存的两倍，不要设到系统盘,硬盘空闲空间大于4.77GB.";
		 String text2="这是我的第二个测试声音";
		 speech1.setFormatType(4);
		// speech.setRate(-10);
		//speech.speak(text); 
		//speech.setRate(-10);
		speech1.saveToWav(text1, "e://test1.wav"); 
		
		
		MSTTSSpeech speech2 = new MSTTSSpeech();
		 speech2.setFormatType(4);
		 speech2.saveToWav(text2, "e://test2.wav");
