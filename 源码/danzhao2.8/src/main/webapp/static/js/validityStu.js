   ...
   $(this).blur(function () {
    ...
    if (_validate) {
     ...
     for (var i = 0; i < arr.length; i++) {
      ...
      if (!check($(this), arr[i], $(this).val().trim()))
       return false;
      else
       continue;
     }
...
var showMsg = function (obj, msg, mark) {
   ...
   if (mark){
   	...
   	var _html = "<span id='errormsg' style='float: left; font-size:13px;color:gray;background:url(" + defaults.img_error + ") no-repeat 0 -1px;padding-left:20px;margin-left:5px;'>" + msg + "</span>";
